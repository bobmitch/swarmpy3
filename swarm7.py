...
resource.path.append('res/audio')
resource.path.append('res/newgfx')
resource.reindex()
...
def update(dt):
	...
	if game.state & game.states['ingame']:
		...
		if not game.state & game.states['paused']:
			...
			#for proj in game.projectiles:
			#	proj.update (dt)
			list(map(lambda t: t.update(dt), game.projectiles))
			#map ( lambda i:i.update (dt), game.projectiles )
		
			#for enemy in game.enemies:
			#	enemy.update(dt)
			list(map(lambda t: t.update(dt), game.enemies))
			#map ( lambda i:i.update (dt), game.enemies )
			
			#for enemy in game.flying_enemies:
			#	enemy.update(dt)
			list(map(lambda t: t.update(dt), game.flying_enemies))
			#map ( lambda i:i.update (dt), game.flying_enemies )
			
			#for tower in game.towers:
			#	tower.update (dt)
			list(map(lambda t: t.update(dt), game.towers))
			#map ( lambda i:i.update (dt), game.towers )
				
			#for particle in game.particles:
			#	particle.update (dt)
			list(map(lambda t: t.update(dt), game.particles))

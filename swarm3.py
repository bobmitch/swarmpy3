...
def draw_text ():
	...
	score_label.y = HEIGHT-50
	credits_label.text = "Credits: " + str(game.credits) 
	credits_label.y = HEIGHT-50
	lives_label.text = "Lives left: " + str(game.lives) 
	lives_label.y = HEIGHT-50
	slow_tower_info_label.x = WIDTH-170
	slow_tower_context_label.x = WIDTH-170
	slow_tower_tooltip_label.x = WIDTH-170
	...
	if amouse.over == ID_SLOWTOWER and selected is None:
		slow_tower_info_label.draw()
	if selected:
		slow_tower_context_label.draw()
	if amouse.over<999 and selected is None:
		slow_tower_tooltip_label.draw()
			...
			if self.type == 0:
				...
				for route in game.map.routes:
					if route[0][0]<1:
						...
						yadda = normalEnemy(newstart, [route[1][0],route[1][1]+offset ], self.health, self.credits )
						yadda.health=self.health
					else:
						# top to bottom route, adjust random x start 
						offset = int(random.random()*6)-3
						newstart=[ route[0][0]+offset, route[0][1] ]
						yadda = normalEnemy(newstart, [route[1][0]+offset,route[1][1]], self.health, self.credits)
						yadda.health=self.health
...
class aMap:
	def __init__ (self, x, y, game):
		...
		self.routes = [ ([0,y/2],[x-1,y/2]) ]	#  list of tuples containing start and end points for swarms
		...
		routepath2 = aPath (self.routes[1][0],self.routes[1][1])
		self.route_paths.append (routepath2)
		
		self.swarms = []	# type, number of critters, health of each critter, time till next swarm
		self.swarms.append ( aSwarm (0, 10, 10, 20, 1) )
		self.swarms.append ( aSwarm (0, 10, 20, 20, 2) )
		self.swarms.append ( aSwarm (0, 10, 40, 20, 3) )
		self.swarms.append ( aSwarm (0, 10, 70, 20, 4) )
		self.swarms.append ( aSwarm (0, 10, 100, 20, 5) )
		self.swarms.append ( aSwarm (0, 10, 200, 20, 10) )
		self.swarms.append ( aSwarm (0, 30, 1000, 20, 20) )
	...
	def recalc_route_paths (self):
		counter=0
		for route in self.routes:
			routepath = aPath (self.routes[0][0],self.routes[0][1])
			self.route_paths[counter] =  copy.copy (routepath)
			counter+=1
	...
	def draw_paths (self):
		...
		for x in range (0,self.width):
			for y in range (0,self.height):
				if self.paths[x][y]:
					glColor4f (1,1,1,self.paths[x][y]*0.1)
					glVertex2f (x,y)
					glVertex2f (x, y+1)
					glVertex2f (x+1, y+1)
					glVertex2f (x+1, y)
...
class normalEnemy:
	""" normal enemy class.  takes:  start, goal, health, credits """
	def __init__ (self, start=[0,8], goal=[30,8], health = 10, credits = 5):
		self.pos = [0,0]
	...
	def update (self, tick):
		if self.health<0:
			# I`m dead
			# remove path from global path counter
			for entry in self.mypath.path:
				game.map.paths[entry[0]][entry[1]]-=1
		...
		if abs(self.pos[0]-self.next_position[0]) < 0.1 and abs(self.pos[1]-self.next_position[1]) < 0.1:
			...
			if self.mypath.path:
				...
				# job done - update all
				game.map.paths[int(self.pos[0])][int(self.pos[1])]-=1	# remove path from global path counter
				self.alive = False
				game.enemies.remove (self)
				game.lives-=1
	...
	if buttons & mouse.LEFT:
		...
		if amouse.over == 999:
			...
			if deploying and amouse.deployable and game.credits >= deploying.cost:
				...
				# check if paths need recalculated
				if game.map.paths[x][y]>0 or game.map.paths[x-1][y]>0 or game.map.paths[x-1][y-1]>0 or game.map.paths[x][y-1]>0:
					# clearing current global path counter
					print "Clearing global path counter..."
					for x in range (0,game.map.width):
						for y in range (0,game.map.height):
							game.map.paths[x][y]=0
					# yes, recalc paths for critters
					print "Recalculating paths..."
					for enemy in game.enemies:
						enemy.mypath.start[0] = int(enemy.pos[0])
...
def draw_filled_circle (r,i, outline=True, alpha=1):
	angle=0
	glColor4f (1,0.2,0.2,0.07*alpha)
...
score_label = pyglet.text.Label("Score: " + str(game.score)  , 
                          font_name='Arial', 
                          font_size=16,
                          x=30, y=window.height-80)
credits_label = pyglet.text.Label("Score: " + str(game.credits)  , 
                          font_name='Arial', 
                          font_size=16,
                          x=430, y=window.height-40)
lives_label = pyglet.text.Label("Score: " + str(game.lives)  , 
                          font_name='Arial', 
                          font_size=16,
                          x=230, y=window.height-40)
slow_tower_info_label = pyglet.text.Label("Slow Tower\nSlow firing but medium damage tower.\n\nCost:		"+str(SLOW_TOWER_COST)+"\nRange:		"+str(SLOW_TOWER_RANGE_SQ)+"\nDamage:	"+str(SLOW_TOWER_DAMAGE), 
                          font_name='Arial', 
                          font_size=11,
                          x=window.width-160, y=window.height-300, italic=True, multiline=True, width=160)
slow_tower_context_label = pyglet.text.Label("Slow Tower\nLVL 1\n\nUpgrade Details:\nRange:		+30%\nDamage:	+25%\n\nUpgrade Cost:	"+str(SLOW_TOWER_UPGRADE_COST)+"\nSale Price:	"+str(SLOW_TOWER_SALE_PRICE), 
                          font_name='Arial', 
                          font_size=11,
                          x=window.width-160, y=window.height-300, italic=True, multiline=True, width=160)
slow_tower_tooltip_label = pyglet.text.Label("Slow Tower\nLVL 1\n\nRange:		"+str(SLOW_TOWER_RANGE_SQ)+"\nDamage:	"+str(SLOW_TOWER_DAMAGE), 
                          font_name='Arial', 
                          font_size=11,
                          x=window.width-160, y=window.height-300, italic=True, multiline=True, width=160)
